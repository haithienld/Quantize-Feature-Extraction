...
class Group(object):
    ...
    def sample(self,match_sample=5,hard_sample=5,non_match=5):
        ...
        try:
            ...
            samples = pd.concat([m1, m2, ss, o], axis=0)
        except:
...
class DataLoader(object):
    def __init__(self,path,embedding_size,image_size):
        self.main_path = os.path.join(path,'image')
        train_list = os.path.join(path,'train-test-split/train_list.txt')
        test_list = os.path.join(path,'train-test-split/test_full.txt')
        self.test_df = pd.read_csv(test_list, sep=" ", header=None)
        self.test_df.columns = ["file", "id", "folder"]
        self.train_df = pd.read_csv(train_list, sep=" ", header=None)
        self.train_df.columns = ["file", "id", "folder"]
        self.image_size = image_size
